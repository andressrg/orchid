...
describe('projectKeyToName', () => {
  ...
  it('preserves hyphens in project names', () => {
    assert.equal(
      projectKeyToName(
        '-Users-juliankmazo-Developer-personal-mining-plate-crawler',
      ),
  ...
  it('falls back to last two segments when no Developer', () => {
    assert.equal(projectKeyToName('-some-random-path'), 'random/path');
  });
});
  ...
  it('computes date range from sessions', () => {
    const sessions = [
      ...
      makeSession({
        sessionId: 's2',
        projectKey: 'p',
        firstTimestamp: '2026-03-01T00:00:00Z',
        lastTimestamp: '2026-04-01T00:00:00Z',
      }),
    ];
    const groups = groupSessionsByProject(sessions);
...
describe('markSessionsSynced', () => {
  it('marks matching sessions as synced', () => {
    ...
    const result = markSessionsSynced({
      sessions,
      syncedIds: new Set(['s1', 's3']),
    });
...
describe('markGroupSessionsSynced', () => {
  it('marks sessions in group and preserves other fields', () => {
    ...
    const result = markGroupSessionsSynced({
      group,
      syncedIds: new Set(['s1']),
    });
...
describe('computeScrollOffset', () => {
  it('keeps scroll when cursor is in viewport', () => {
    assert.equal(
      computeScrollOffset({ cursor: 5, scroll: 0, maxVisible: 10, total: 20 }),
      0,
    );
  });
  ...
  it('scrolls up when cursor goes above viewport', () => {
    assert.equal(
      computeScrollOffset({ cursor: 2, scroll: 5, maxVisible: 10, total: 20 }),
      2,
    );
  });
  ...
  it('scrolls down when cursor goes below viewport', () => {
    assert.equal(
      computeScrollOffset({ cursor: 15, scroll: 0, maxVisible: 10, total: 20 }),
      6,
    );
  });
  ...
  it('does not scroll past end', () => {
    assert.equal(
      computeScrollOffset({ cursor: 19, scroll: 0, maxVisible: 10, total: 20 }),
      10,
    );
  });
  ...
  it('handles cursor at position 0', () => {
    assert.equal(
      computeScrollOffset({ cursor: 0, scroll: 5, maxVisible: 10, total: 20 }),
      0,
    );
  });
  ...
  it('handles single-page list (no scrolling needed)', () => {
    assert.equal(
      computeScrollOffset({ cursor: 3, scroll: 0, maxVisible: 10, total: 5 }),
      0,
    );
  });
});
