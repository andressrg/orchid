...
): readonly JsonValue[] => (Array.isArray(hooks[event]) ? hooks[event] : []);
...
const assertHookCommand = (
  command: string | null,
  subcommand: '_on-start' | '_on-stop' | '_on-end',
): void => {
...
describe('buildHookEntries', () => {
  it('returns only Claude hook event keys', () => {
    ...
    assert.deepEqual(
      keys.filter(
        (key) =>
          !ORCHID_HOOK_EVENTS.includes(
            key as (typeof ORCHID_HOOK_EVENTS)[number],
          ),
      ),
      [],
    );
  ...
  it('installs lifecycle hooks for current Claude sessions', () => {
    ...
    const startEntry = objectFromValue(
      entriesForEvent(entries, 'SessionStart')[0],
    );
  ...
  it('uses the internal hook commands', () => {
    ...
    assertHookCommand(
      firstCommandForEvent(entries, 'SessionStart'),
      '_on-start',
    );
...
describe('isOrchidHookEntry', () => {
  it('detects legacy orchid hook entries', () => {
    ...
    assert.equal(
      isOrchidHookEntry({
        hooks: [
          { type: 'command', command: 'orchid hooks _on-stop', timeout: 30 },
        ],
      }),
      true,
    );
    ...
    assert.equal(
      isOrchidHookEntry({
        hooks: [
          {
            type: 'command',
            command: "'/Users/example/.orchid/hooks/orchid-hook' _on-start",
          },
        ],
      }),
      true,
    ...
    assert.equal(
      isOrchidHookEntry({
        hooks: [
          {
            type: 'command',
            command: "'/Users/example/.orchid/hooks/orchid-hook' _on-stop",
          },
        ],
      }),
      true,
    ...
    assert.equal(
      isOrchidHookEntry({
        ...
        hooks: [
          {
            type: 'command',
            command: "'/Users/example/.orchid/hooks/orchid-hook' _on-end",
          },
        ],
      }),
      true,
...
describe('mergeHooks', () => {
  ...
  it('merges overlapping Stop hooks', () => {
    ...
    assert.equal(
      firstCommandForEvent({ Stop: [stopEntries[0]] }, 'Stop'),
      'echo done',
    );
    assertHookCommand(
      firstCommandForEvent({ Stop: [stopEntries[1]] }, 'Stop'),
      '_on-stop',
    );
  });
  ...
  it('replaces legacy PATH-based orchid entries', () => {
    ...
    const startEntry = objectFromValue(
      entriesForEvent(result, 'SessionStart')[0],
    );
    ...
    assert.equal(startEntry.matcher, 'startup|resume|clear|compact');
    assertHookCommand(
      firstCommandForEvent(result, 'SessionStart'),
      '_on-start',
    );
  });
});
