...
const FAKE_TRANSCRIPT = [
  ...
  JSON.stringify({ type: "assistant", content: "I received your test message." }),
  JSON.stringify({ type: "human", content: "Just verifying the E2E flow works." }),
...
async function testPATAuth(): Promise<void> {
  ...
  assert(healthRes.status === 200, `Health returns 200 (got ${healthRes.status})`);
  ...
  assert(noAuthRes.status === 401, `No auth returns 401 (got ${noAuthRes.status})`);
  ...
  assert(badAuthRes.status === 401, `Invalid token returns 401 (got ${badAuthRes.status})`);
  ...
  const putBody = (await putRes.json()) as { id: string; message_count: number; user_id: string };
  assert(putBody.id === TEST_SESSION_ID, "Session ID matches");
  assert(putBody.message_count === 4, `Message count is 4 (got ${putBody.message_count})`);
  ...
  const getRes = await fetch(`${BASE}/sessions/${TEST_SESSION_ID}`, { headers: AUTH_HEADERS });
  ...
  assert(!!searchResults.find((s) => s.id === TEST_SESSION_ID), "Search finds session");
  ...
  const validateRes = await fetch(`${BASE}/tokens/validate`, { headers: AUTH_HEADERS });
  assert(validateRes.status === 200, `Validate returns 200 (got ${validateRes.status})`);
  const validate = (await validateRes.json()) as { valid: boolean; userId: string };
  ...
  // 404
  const verifyRes = await fetch(`${BASE}/sessions/${TEST_SESSION_ID}`, { headers: AUTH_HEADERS });
