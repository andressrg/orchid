...
test('parseTranscriptTurns parses Claude user and assistant messages', () => {
...
test('parseTranscriptTurns parses Codex canonical user and assistant events without duplicates', () => {
  const transcript = jsonl(
    {
      type: 'session_meta',
  ...
  assert.equal(countMeaningfulTranscriptTurns(transcript), 2);
});

test('parseTranscriptTurns parses Codex tool calls and outputs', () => {
  const transcript = jsonl(
    {
      type: 'response_item',
...
test('summarizeTranscriptMetadata extracts Codex metadata', () => {
  const transcript = jsonl({
    type: 'session_meta',
    timestamp: '2026-05-01T00:00:00.000Z',
    payload: {
  ...
  assert.deepEqual(summarizeTranscriptMetadata({ transcript }), {
    id: 'codex-session',
    cwd: '/repo',
    branch: 'main',
    timestamp: '2026-05-01T00:00:00.000Z',
