...
const parseClaudeTurn = (entry: JsonRecord): TranscriptTurn | null => {
  ...
  const rawRole = asString(entry.role) ?? asString(message?.role) ?? asString(entry.type);
  const role =
...
const dedupeCodexTurns = (turns: readonly TranscriptTurn[]): readonly TranscriptTurn[] =>
  turns.reduce<readonly TranscriptTurn[]>((acc, turn) => {
    const key = meaningfulTurnKey(turn);
    const existingIndex = acc.findIndex((existing) => meaningfulTurnKey(existing) === key);
    if (existingIndex === -1) return [...acc, turn];
    if (!shouldPreferCodexTurn(turn)) return acc;
    return acc.map((existing, index) => (index === existingIndex ? turn : existing));
  }, []);

const detectTranscriptFormat = (entries: readonly JsonRecord[]): TranscriptFormat =>
  entries.some(
    (entry) =>
      entry.type === 'session_meta' || entry.type === 'event_msg' || entry.type === 'response_item',
  )
    ? 'codex-rollout-jsonl'
    : 'claude-jsonl';
...
export const summarizeTranscriptMetadata = (params: {
  ...
  const sessionMeta = entries
    .map((entry) => (entry.type === 'session_meta' ? asRecord(entry.payload) : null))
    .find((payload) => payload !== null);
  const turnContext = entries
    .map((entry) => (entry.type === 'turn_context' ? asRecord(entry.payload) : null))
    .find((payload) => payload !== null);
