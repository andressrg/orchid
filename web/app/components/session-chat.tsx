  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center gap-2 mb-1.5">
        ...
        <span className="text-[11px] font-medium text-night-400">{isUser ? 'You' : 'Orchid'}</span>
      </div>
      <div
...
export function SessionChat({ sessionId }: { sessionId: string }) {
  ...
  async function sendMessage(question: string) {
    ...
    try {
      const res = await fetch(`${API_URL}/api/sessions/${encodeURIComponent(sessionId)}/chat`, {
        method: 'POST',
        headers: {
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pb-4">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full">
            ...
            <p className="text-[14px] font-medium mb-1 text-night-100">Ask about this session</p>
            <p className="text-[12px] mb-6 text-center max-w-sm text-night-400">
              Ask questions about the conversation and Orchid will reason through the transcript to
              find answers.
            </p>
