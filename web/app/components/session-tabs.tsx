...
const TABS = [
  { id: "conversation" as const, label: "Conversation", icon: (
  ...
  { id: "chat" as const, label: "Ask", icon: (
    <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
    ...
    <div>
      ...
      <div className="flex items-center gap-0 px-6 border-b border-night-750 bg-night-900">
        {TABS.map((tab) => (
          ...
          >
            <span className={activeTab === tab.id ? 'text-orchid' : ''}>
              {tab.icon}
            </span>
            {tab.label}
            {activeTab === tab.id && (
      ...
      {activeTab === "commits" && (
        <SessionCommits sessionId={sessionId} />
      )}
